...
Skudar::Source::Source(float alpha, float beta, int sourcesAmount)
{
    this->alpha = alpha;
    this->beta = beta;
...
Skudar::Request Skudar::Source::getNextRequest()
{
    fillTimesToWait();
...
void Skudar::Source::fillTimesToWait()
{
    for (int i = 0; i < sourcesAmount; i++) {
        if (timesToWait[i] <= 0) {
            requestNumbers[i]++;
...
std::pair<int, float> Skudar::Source::getMinTimeAndIndex()
{
...
void Skudar::Source::removeMinTimeToWait()
{
    std::pair<int, float> min = getMinTimeAndIndex();
    for (int i = 0; i < sourcesAmount; i++) {
